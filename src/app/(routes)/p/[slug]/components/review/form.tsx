...
interface FormWrapperPorps {
  reviews: ReviewWithAuthor[];
  children: React.ReactNode;
}

const FormWrapper: FC<FormWrapperPorps> = ({ reviews, children }) => {
  return (
    <>
...
const ReviewForm: FC<ReviewFormProps> = ({ id, me, myReview, reviews }) => {
  ...
  const [optimisticReviews, addOptimisticReviews] = useOptimistic(
    reviews,
    (state, newReview: ReviewWithAuthor) => {
  ...
  const onSubmit = async (data: ReviewFormValues) => {
    try {
      ...
      addOptimisticReviews(newReview);
  ...
  if (optimisticMyReview) {
    return (
      <FormWrapper reviews={optimisticReviews}>
        <div className="mt-8">
